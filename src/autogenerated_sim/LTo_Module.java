...
public class LTo_Module extends SimplePortrayal2D implements Steppable, Stoppable {
...
public LTo_Step getLTo_Module_LTo_Step()
{
	...
	public void step(SimState state)
	{
		...
		if(state.schedule.getSteps() < (Mason_Sim_Main.simParams.simulationTime))
		{
			if(!this.stopped)
    		{
				...
				//if(Mason_Sim_Main.trace_files_output)
				//{
				//	try {
				//		writer.write(ppsim.schedule.getSteps()+","+getLTo_Module_LTo_Step().lTinContactStateChangeTimePoint+
				//				","+getLTo_Module_LTo_Step().lTiContactStateChangeTimePoint+","+getLTo_Module_LTo_Step().adhesionExpressed+
				//				","+getLTo_Module_LTo_Step().chemokineExpressed+"\n");
				//		writer.flush();
				//	} catch (IOException e) {
				//		e.printStackTrace();
				//	}
    ...
    {		
		double diamx = Mason_Sim_Main.display.getScale()*Mason_Sim_Main.simParams.LTO_DIAMETER;
        double diamy = Mason_Sim_Main.display.getScale()*Mason_Sim_Main.simParams.LTO_DIAMETER;
    	   
        if(!this.getLTo_Module_LTo_Step().LTo_Step_LTo_Attributes.expressingRET)
